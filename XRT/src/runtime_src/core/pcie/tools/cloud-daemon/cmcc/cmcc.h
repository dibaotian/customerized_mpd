...
/*
 * This class is for cmcc xclbin download handling.
 *
 * Cmcc uses a http wireserver, which provides RESTful APIs, to handle xclbin
...
{
public:
    explicit CmccDev(size_t index);
    ~CmccDev();

    // Bitstreams
    //int cmccLoadXclBin(const xclBin *buffer);
    int cmccLoadXclBin(const xclBin *buffer, size_t index);
    int cmccHotReset();
...
private:
    ...
    void get_fpga_BDF(unsigned index, std::string &fpgaBDF);
    void get_kernel_uuid(std::string &kernel_uuid);
    void execProgram(const char *cmd, char *result);  
};
